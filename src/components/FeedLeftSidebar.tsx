...
const suggestedPeople = [
  { name: 'Steve Jobs', role: 'CEO of Apple', image: people1Image },
  { name: 'Ryan Roslansky', role: 'CEO of Linkedin', image: people2Image },
  { name: 'Dylan Field', role: 'CEO of Figma', image: people3Image },
];

const events = [
  { date: '10', month: 'Jul', title: 'No more terrorism no more cry' },
  { date: '10', month: 'Jul', title: 'No more terrorism no more cry' },
];
  ...
  return (
    <div className='space-y-4 pb-4'>
      <section className='feed-theme-card rounded-md border border-[#e7eaf4] bg-white px-6 py-6'>
        <h4 className='mb-6 text-base font-semibold leading-6 text-[#1f2a44]'>
          Explore
        </h4>
        <ul className='space-y-4'>
          {exploreItems.map((item) => (
            <li key={item.label}>
              <a
                href='#0'
                className='flex items-center justify-between text-sm font-normal text-[#666] transition hover:text-[#3f4f71]'
              >
                <span className='flex items-center gap-2.5'>
                  <span className='inline-flex shrink-0 items-center justify-center [&>svg]:h-5 [&>svg]:w-5'>
                    {item.icon}
                  </span>
                  <span className='hover:text-blue-400'>{item.label}</span>
                </span>
                {item.isNew ? (
                  <span className='inline-flex h-5 w-8 items-center justify-center rounded-[8px] border-2 border-white bg-[#0ACF83] text-xs leading-[1.3] font-normal text-white'>
                    New
                  </span>
                ) : null}
              </a>
      ...
      <section className='feed-theme-card rounded-md border border-[#e7eaf4] bg-white px-6 py-6'>
        ...
        <div>
          {suggestedPeople.map((person, index) => (
            <div
              key={person.name}
              className={`flex items-center justify-between gap-2 ${index === suggestedPeople.length - 1 ? '' : 'mb-6'}`}
            >
              <div className='flex flex-1 items-center'>
                <img
                  src={person.image}
                  alt={person.name}
                  className={`mr-4 rounded-full object-cover ${index === 0 ? 'h-10 w-10' : 'h-[37px] w-[37px]'}`}
                />
                <div className='flex-1'>
                  <p className='text-[14px] leading-[1.1] font-medium text-[#212121]'>
                    {person.name}
                  </p>
                  <p className='text-[11px] leading-[1.4] font-light text-[#212121]'>
                    {person.role}
                  </p>
                </div>
              </div>
              ...
              <button
                type='button'
                className='block rounded-[2px] border border-[#DCDFE4] bg-white px-[7px] py-[7px] text-[12px] leading-[1.4] font-medium text-[#959EAE] transition hover:border-[#1890FF] hover:bg-[#1890FF] hover:text-[#F3F9FF]'
              >
                Connect
              </button>
            </div>
      ...
      <section className='feed-theme-card rounded-md border border-[#e7eaf4] bg-white px-6 pt-6 pb-1.5'>
        <div className='mb-6 flex items-center justify-between'>
          <h4 className='text-base font-semibold text-[#1f2a44]'>Events</h4>
          <a
            href='#0'
            className='block px-[1px] py-[5px] text-xs leading-[18px] font-normal text-[#1890FF]'
          >
            See all
          </a>
        </div>
        ...
        <div>
          {events.map((event, index) => (
            <a
              ...
              href='#0'
              className='mb-4 block cursor-pointer rounded-[6px] bg-white shadow-[0px_4px_8px_rgba(0,0,0,0.08)]'
            >
              <img
                src={feedEventImage}
                alt={event.title}
                className='h-36 w-full rounded-[6px] object-cover'
              />
              <div className='flex items-center px-4 pt-5 pb-[14px]'>
                <div className='w-fit rounded-[2px] bg-[#0ACF83] p-2 text-center'>
                  <p className='text-[18px] leading-[1.1] font-bold text-white'>
                    {event.date}
                  </p>
                  <p className='text-[18px] leading-[1.1] font-normal text-white'>
                    {event.month}
                  </p>
                </div>
                <h5 className='pl-2 pt-[5px] text-[16px] leading-[1.4] font-medium text-black'>
                  {event.title}
                </h5>
              </div>
              ...
              <div className='flex items-center justify-between px-4 py-3'>
                <span className='text-xs leading-[18px] font-medium text-[#8A8A8A]/70'>
                  17 People Going
                </span>
                <span className='block rounded-[2px] border border-[#1890FF] bg-[#F3F9FF] px-[14px] py-[3px] text-xs leading-[18px] font-medium text-[#1890FF] transition hover:bg-[#1890FF] hover:text-[#F3F9FF]'>
                  Going
                </span>
              </div>
            </a>
